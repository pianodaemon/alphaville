...
//var parseArgs = require("minimist");
var messages = require("./grpc/patios_pb");
var services = require("./grpc/patios_grpc_pb");
const TARGET =
  process.env.NODE_ENV === "production"
    ? "neon_nights:10080"
    : "localhost:10080";
const PROTO_PATH = __dirname + "/grpc/protos/patios.proto";
const options = {
  keepCase: true,
  longs: String,
  enums: String,
...
const PatioService = grpc.loadPackageDefinition(packageDefinition).dylk.Patios;

function setupPatiosClient(method) {
  // console.log(TARGET, process.env.NODE_ENV);
  const client = new PatioService(TARGET, grpc.credentials.createInsecure());
  return (promisifiedClient = promisify(client[method]).bind(client));
}

async function listPatios(query) {
  try {
    const promisifiedClient = setupPatiosClient("listPatios");
...
async function createPatio(fields) {
  const promisifiedClient = setupPatiosClient("alterPatio");
  const patio = {
    id: 0,
    code: fields.code,
    title: fields.title,
...
async function readPatio(id) {
  const promisifiedClient = setupPatiosClient("getPatio");
  const call_service = async (req, service_name) => {
    try {
      // @todo add SearchParam (filters)
      // var param = new messages.Param().setName('disabled').setValue(false);
...
async function updatePatio(id, fields) {
  const promisifiedClient = setupPatiosClient("alterPatio");
  const patio = {
    id,
    code: fields.code,
...
async function deletePatio(id) {
  const promisifiedClient = setupPatiosClient("deletePatio");
  const call_service = async (req, service_name) => {
    try {
      // @todo add SearchParam (filters)
      // var param = new messages.Param().setName('disabled').setValue(false);
